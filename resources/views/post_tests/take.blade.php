...
@section('content')
    ...
    <div class="fixed inset-0 pointer-events-none z-0">
        <div class="absolute top-[15%] left-[5%] w-32 h-32 lg:w-40 lg:h-40 bg-white bg-opacity-5 rounded-full animate-float">
        </div>
        <div class="absolute top-[60%] right-[8%] w-20 h-20 lg:w-25 lg:h-25 bg-white bg-opacity-5 rounded-full animate-float"
            style="animation-delay: 3s;"></div>
        <div class="absolute bottom-[25%] left-[15%] w-16 h-16 lg:w-20 lg:h-20 bg-white bg-opacity-5 rounded-full animate-float"
                        ...
                        <div class="flex flex-col lg:flex-row justify-between items-center gap-8">
                            ...
                            <div class="text-center lg:text-left">
                                <div class="flex items-center justify-center lg:justify-start gap-3 mb-3">
                                    ...
                                    <span
                                        class="bg-white bg-opacity-20 backdrop-blur-lg px-3 py-1 rounded-full text-sm font-medium">POST
                                        TEST</span>
                                </div>
                    ...
                    @if (!isset($attempt) || !$attempt)
                        ...
                        <div class="bg-white rounded-2xl p-8 mb-8 shadow-2xl">
                            ...
                            @if ($postTest->description)
                                <div
                                    class="flex items-center gap-3 p-4 bg-gradient-to-r from-amber-50 to-orange-50 rounded-xl mb-6 text-slate-700">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                                <!-- Questions Stat -->
                                <div
                                    class="flex items-center gap-4 p-4 bg-gradient-to-r from-slate-50 to-gray-50 rounded-xl border-l-4 border-gray-600 hover:transform hover:-translate-y-1 transition-all duration-300 hover:shadow-lg">
                                    <div
                                        class="w-10 h-10 bg-posttest-primary-gradient rounded-xl flex items-center justify-center text-white">
                                        <i class="fas fa-list-ol"></i>
                                    </div>
                                    <div>
                                        <div class="text-2xl font-bold text-slate-800">{{ $postTest->questions->count() }}
                                        </div>
                                ...
                                <!-- Time Stat -->
                                <div
                                    class="flex items-center gap-4 p-4 bg-gradient-to-r from-slate-50 to-red-50 rounded-xl border-l-4 border-red-600 hover:transform hover:-translate-y-1 transition-all duration-300 hover:shadow-lg">
                                    <div
                                        class="w-10 h-10 bg-posttest-secondary-gradient rounded-xl flex items-center justify-center text-white">
                                ...
                                <!-- Passing Score Stat -->
                                <div
                                    class="flex items-center gap-4 p-4 bg-gradient-to-r from-slate-50 to-emerald-50 rounded-xl border-l-4 border-emerald-600 hover:transform hover:-translate-y-1 transition-all duration-300 hover:shadow-lg">
                                    <div
                                        class="w-10 h-10 bg-posttest-success-gradient rounded-xl flex items-center justify-center text-white">
                            ...
                            <!-- Important Notice -->
                            <div
                                class="bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-red-500 p-6 rounded-xl mb-8">
                            ...
                            <div class="text-center">
                                <form action="{{ route('post_tests.start', $postTest) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="inline-flex items-center justify-center gap-3 px-12 py-4 bg-posttest-primary-gradient text-white rounded-xl font-bold text-lg shadow-xl hover:transform hover:-translate-y-1 hover:shadow-2xl transition-all duration-300">
                    ...
                    @else
                        ...
                        @if (isset($attempt) && $attempt)
                            <div class="bg-white rounded-2xl p-6 mb-8 shadow-xl">
                                <div
                                    class="flex items-center gap-3 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl text-blue-800">
                                    <i class="fas fa-info-circle text-blue-600"></i>
                                    <div>
                                        <strong>Melanjutkan Post Test</strong> - Anda memiliki sesi yang sedang berjalan.
                                        <br><small class="text-blue-600">Waktu tersisa: <span id="resume-time"
                                                class="font-mono font-bold">{{ gmdate('i:s', $attempt->time_remaining) }}</span></small>
                                    </div>
                                </div>
                            </div>
                        @endif
                        ...
                        <!-- Post Test Form -->
                        <form id="posttest-form" action="{{ route('post_tests.submit', $postTest) }}" method="POST"
                                        ...
                                        @if ($question->image)
                                            <div class="mb-6">
                                                <img src="{{ asset('storage/' . $question->image) }}" alt="Question Image"
                                                    class="max-w-full h-auto rounded-xl border-2 border-gray-100 shadow-lg max-h-96 mx-auto">
                                            </div>
                                        @endif
                                            ...
                                            @foreach ($question->options as $optionIndex => $option)
                                                <div class="relative">
                                                    <input class="absolute opacity-0 cursor-pointer option-radio"
                                                        ...
                                                        value="{{ $optionIndex }}" data-question-id="{{ $question->id }}"
                                                        {{ isset($attempt->answers[$question->id]) && $attempt->answers[$question->id] == $optionIndex ? 'checked' : '' }}>
                                                    <label
                                                        class="option-label flex items-center gap-4 p-5 border-2 border-gray-200 rounded-xl cursor-pointer bg-gray-50 hover:border-gray-600 hover:bg-white hover:shadow-lg transition-all duration-300 relative overflow-hidden group {{ isset($attempt->answers[$question->id]) && $attempt->answers[$question->id] == $optionIndex ? 'border-gray-600 bg-white shadow-lg selected-option' : '' }}"
                                                        for="q{{ $question->id }}_option{{ $optionIndex }}">
                                                        <div
                                                            class="absolute inset-0 bg-posttest-primary-gradient opacity-0 group-hover:opacity-10 transition-opacity duration-300">
                                                        </div>
                                                        <div
                                                            class="option-marker w-9 h-9 bg-gray-300 rounded-full flex items-center justify-center font-bold text-gray-600 group-hover:bg-gray-600 group-hover:text-white transition-all duration-300 z-10 {{ isset($attempt->answers[$question->id]) && $attempt->answers[$question->id] == $optionIndex ? 'bg-gray-600 text-white transform scale-105' : '' }}">
                                                            {{ chr(65 + $optionIndex) }}
                                                        </div>
                                                        <div class="flex-1 text-slate-700 z-10">{{ $option }}</div>
                                                        <div
                                                            class="option-check w-6 h-6 bg-gray-300 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 z-10 {{ isset($attempt->answers[$question->id]) && $attempt->answers[$question->id] == $optionIndex ? 'bg-green-600 opacity-100 transform scale-110' : '' }}">
                                ...
                                <div class="flex flex-col lg:flex-row items-center justify-between gap-4">
                                    <div class="flex-1 w-full lg:w-auto">
                                        <div class="flex items-center gap-4 mb-2">
                                            ...
                                            <div id="progress-text" class="text-sm font-bold text-gray-800">
                                                0/{{ $postTest->questions->count() }} terjawab</div>
                                        </div>
                                        <div class="w-full bg-gray-200 rounded-full h-3">
                                            <div id="progress-bar"
                                                class="bg-posttest-primary-gradient h-3 rounded-full transition-all duration-300"
                                                style="width: 0%"></div>
                                        </div>
                                    </div>
                                    <button type="button" id="submit-posttest-btn"
                                        class="w-full lg:w-auto inline-flex items-center justify-center gap-3 px-8 py-3 bg-posttest-primary-gradient text-white rounded-xl font-bold shadow-xl hover:transform hover:-translate-y-1 hover:shadow-2xl transition-all duration-300">
        ...
        <div id="confirmationModal" class="fixed inset-0 z-50 opacity-0 invisible transition-all duration-300">
            ...
            <div class="relative flex items-center justify-center min-h-screen p-4">
                <div
                    ...
                    <!-- Modal Header -->
                    <div
                        class="bg-posttest-primary-gradient text-white p-6 rounded-t-2xl flex items-center justify-between">
                    ...
                    <div class="p-6">
                        <p class="text-gray-700 mb-4">
                            ...
                            <strong class="text-red-600">Setelah dikirim, Anda tidak dapat mengubah jawaban lagi dan ini
                                akan mempengaruhi nilai akhir Anda.</strong>
                        </p>

                        <div id="unanswered-alert"
                            class="hidden bg-gradient-to-r from-red-50 to-orange-50 border-l-4 border-red-500 p-4 rounded-xl">
                    ...
                    <div class="flex gap-3 p-6 border-t border-gray-100">
                        ...
                        <button type="button" id="confirm-submit-btn"
                            class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-3 bg-posttest-primary-gradient text-white rounded-xl font-semibold hover:opacity-90 transition-opacity">
    ...
    <style>
        ...
        .bg-posttest-primary-gradient {
            background: linear-gradient(135deg, #2d3748 0%, #4a5568 100%);
        }

        .bg-posttest-secondary-gradient {
            background: linear-gradient(135deg, #e53e3e 0%, #c53030 100%);
        }

        .bg-posttest-success-gradient {
            background: linear-gradient(135deg, #38a169 0%, #2f855a 100%);
        }

        .bg-posttest-gradient {
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                document.addEventListener('keydown', function(e) {
                    if (e.key === 'Escape' && confirmationModal && !confirmationModal.classList.contains(
                            'invisible')) {
                        hideConfirmationModal();
                    }
                });
                ...
                window.addEventListener('beforeunload', function(e) {
                    ...
                    if (!isSubmitting && timeRemaining > 0) {
                        ...
                        e.preventDefault();
                        e.returnValue =
                            'Apakah Anda yakin ingin meninggalkan halaman? Progress Post Test akan hilang.';
                    }
                });
