    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside id="sidebar"
            ...
            <div class="flex items-center justify-between p-6 border-b border-white/10">
                ...
                <button id="closeSidebar" class="lg:hidden text-gray-400 hover:text-white p-2 rounded-lg hover:bg-white/10 transition-colors">
            ...
            <nav class="flex-1 px-4 py-6 custom-scrollbar overflow-y-auto">
                <div class="space-y-6">
                    ...
                    <!-- Main Navigation -->
                    <div>
                        <h3 class="px-4 py-2 text-xs font-semibold text-purple-200 uppercase tracking-wider">Main Menu</h3>
                    ...
                    @if ($teacherClassesWithRequests && $teacherClassesWithRequests->count() > 0)
                        <div>
                            <h3 class="px-4 py-2 text-xs font-semibold text-purple-200 uppercase tracking-wider">Permintaan Mentor</h3>
                            <ul class="space-y-1 mt-3">
                                @foreach ($teacherClassesWithRequests as $teacherClass)
                                    ...
                                    <li>
                                        <a href="{{ $href }}"
                                            ...
                                            <span class="bg-red-500 text-white text-xs px-2 py-1 rounded-full font-semibold min-w-[24px] text-center">
                    ...
                    @if (auth()->user()->teacherClasses && auth()->user()->teacherClasses->count() > 0)
                        <div>
                            <h3 class="px-4 py-2 text-xs font-semibold text-purple-200 uppercase tracking-wider">Kelas Mentor</h3>
                            <ul class="space-y-1 mt-3">
                                @foreach (auth()->user()->teacherClasses as $teacherClass)
                                    <li>
                                        <a href="{{ route('teacher-classes.implementation', $teacherClass) }}"
                                            ...
                                            <span class="bg-blue-500 text-white text-xs px-2 py-1 rounded-full font-semibold min-w-[24px] text-center">
            ...
            <header class="bg-white shadow-sm border-b border-gray-200 px-6 py-4">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <!-- Profile Dropdown -->
                        <div class="relative">
                            <button id="profileButton" class="flex items-center space-x-3 p-2 rounded-lg hover:bg-gray-100 transition-colors">
                                <img src="{{ auth()->user()->avatar ? asset('storage/' . auth()->user()->avatar) : 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) }}"
                                     alt="{{ auth()->user()->name }}" class="w-10 h-10 rounded-full border-2 border-gray-200">
                            ...
                            <!-- Dropdown Menu -->
                            <div id="profileDropdown" class="profile-dropdown absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-200 py-2 z-50">
